...
class If_Else_AST:
    ...
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
                       repr(self.then) + repr(self.els) + ' end'
    def indented(self, level):
...
class Boolean_Not_AST:
    ...
    def __repr__(self):
        return self.op + repr(self.right)
...
class Boolean_Or_AST:
    def __init__(self, term1, term2):
        self.term1 = term1
        self.term2 = term2
        self.op = 'or'
    def __repr__(self):
        return repr(self.term1) + self.op + repr(self.term2)
    ...
    def indented(self, level):
        return indent(self.op, level) + \
               ...
               self.right.indented(level + 1)
    def true_code(self, label):
        return self.term1.true_code(label) + \
               self.term2.true_code(label)

    def false_code(self, label):
        l1 = label_generator.next()
        return self.term1.true_code(l1) + \
               self.term2.false_code(label) + \
               l1 + ':\n'
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        return If_Else_AST(condition, then, els)    
...
def boolean_expression():
    ...
    while scanner.lookahead() == Token.OR:
        ...
        tree = boolean_term()
        result = Boolean_Or_AST(result, tree)
    return result
...
def boolean_term():
    ...
    while scanner.lookahead() == Token.AND:
        ...
        tree = boolean_factor()
        result = Boolean_And_AST(result, tree)
    return result
